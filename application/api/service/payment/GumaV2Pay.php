...
//        Log::error('接口商户：'.json_encode($user,true));
        ...
        $orderId = Db::name('ewm_order')->where('order_no',$params['trade_no'])->value('id');
        $data['is_bzk'] = $is_bzk;
...
//            $data['gourl'] = 'alipays://platformapi/startapp?appId=68687093&url='.urlencode($url);
        ...
        }elseif ($codes == 'alipayCode'){
            return config('zhongzhuan_url').'alipaycode.php?'. http_build_query($data);
        }
//        $paofenPayUrl = db('config')->where(['name' => 'thrid_url_gumapay'])->value('value');;
       if($type==30)
       {
           $kzk_is_pay_name = Db::name('config')->where(['admin_id'=>$user['admin_id'],'name'=>'kzk_is_pay_name'])->value('value');
           if (empty($kzk_is_pay_name)){
               $data['kzk_is_pay_name'] = 1;
           }else{
               $data['kzk_is_pay_name'] = $kzk_is_pay_name;
           }
           $data['money'] = $money;
	       $r['money'] =  $money;
