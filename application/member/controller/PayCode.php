...
class PayCode extends Base
{
    ...
    public function editXiane(){
        ...
        $codeInfo = $this->modelEwmPayCode->where(['ms_id'=>$this->agent_id, 'id' => $code_id])->find();
        if (!$codeInfo){
            $this->error('非法操作');
        }
	...
	if(strlen( $account_name<10)){
        ...
        $codeSum = Db::name('ewm_pay_code')->where(['code_type'=>30,'is_delete'=>0,'ms_id'=>$this->agent_id])->count('id');
        $codeOnSum = Db::name('ewm_pay_code')->where(['code_type'=>30,'is_delete'=>0,'ms_id'=>$this->agent_id,'status'=>1])->count('id');
...
//        print_r($data);die;
            $result = $codeLogic->addQRcode($data,30);
        ...
        $pay_code = Db::name('pay_code')->where('status',1)->field('id,name')->select();
