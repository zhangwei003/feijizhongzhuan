;(function () {
    ...
    // bootstrap年选择器
    var bootstrapDateInput = $("input.js-bootstrap-year")
    ...
    // bootstrap年份选择器
    var bootstrapYearTimeInput = $("input.js-bootstrap-year");
    if (bootstrapYearTimeInput.length) {
        Wind.css('bootstrapDatetimePicker');
        Wind.use('bootstrapDatetimePicker', function () {
            bootstrapYearTimeInput.datetimepicker({
                language: 'zh-CN',
                viewMode: 'year',
                startView: 'decade',
                minView: 'decade',
    ...
    if ($js_address_select.length > 0) {
        $('.js-address-nation-select,.js-address-province-select,.js-address-city-select').change(function () {
            var $this                   = $(this);
            var id                      = $this.val();
            var $child_area_select;
            var $this_js_address_select = $this.parents('.js-address-select');
            if ($this.is('.js-address-nation-select')) {
                $child_area_select = $this_js_address_select.find('.js-address-province-select');
                $this_js_address_select.find('.js-address-province-select').hide();
...
function uploadOneImage(dialog_title, input_selector, extra_params, app) {
    //$("#input_selector").val(input_selector);
    openUploadDialog(dialog_title, function (dialog, files) {
        $(input_selector).val(files[0].filepath);
 ...
 * @param img 图片地址
 */
function selectOneImage(img, preview_url, filename, input_selector = '#thumbnail') {
    $(input_selector).val(img);
    $(input_selector + '-preview').attr('src', preview_url);
    $(input_selector + '-name').val(filename);
}
